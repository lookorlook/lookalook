...
def _count_qualifying_supplement_shifts(att_records, name):
    ...
    for r in att_records:
        ...
        total_hours += r.hours
        # Shifts starting at 6h, 11h, or 14h qualify
        if start_h in (6, 11, 14):
            qualifying_count += 1
...
def apply_tempoteam_rules(attendance_sheet, invoice_emps_dict):
    ...
    for can_name, inv_data in invoice_emps_dict.items():
        ...
        # --- Supplement check ---
        supp_check = None
        supp_equipe_item = _get_invoice_item(inv_items, "Suppl\u00e9ment \u00e9quipe") or _get_invoice_item(inv_items, "Supplement")
        qty_count, qty_hours, total_hours = _count_qualifying_supplement_shifts(
            attendance_sheet.records, att_name)
        
        if supp_equipe_item:
            inv_supp_hours = supp_equipe_item.qty
            if qty_count > 0:
                diff_h = abs(qty_hours - inv_supp_hours)
                supp_check = {
                    "status": "ok" if diff_h <= 0.5 else "hours_mismatch",
                    "att_hours": round(qty_hours, 2),
                    "inv_hours": round(inv_supp_hours, 2),
                    "qualifying_shifts": qty_count,
                }
            else:
                supp_check = {
                    "status": "unexpected",
        ...
        if dimona_item:
            expected_dimona_qty = sum(i.qty for i in inv_items
                                       if i.name in ["Prestations", "Hrs suppl\u00e9m. pay\u00e9es imm\u00e9diatement"])
            dimona_check = {
